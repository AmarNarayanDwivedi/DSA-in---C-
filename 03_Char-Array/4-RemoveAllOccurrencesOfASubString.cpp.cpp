...
public:
    string removeOccurrences(string s, string part)
    {
        ...
        int indexOfPattern = s.find(part);
        int lengthOfPattern = part.length();

        while (indexOfPattern != string::npos)
        {
            s.erase(indexOfPattern, lengthOfPattern);
            indexOfPattern = s.find(part);
        }
        return s;
    }
};